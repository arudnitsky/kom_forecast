...
def calculate_overall_segment_favorability(
    segment_deg: int,
    wind_data: Dict[str, Any],
    min_wind_speed: float = 8.0,
    direction_tolerance: int = 45,
) -> float:
...
def format_wind_conditions(conditions: WindConditions, segment_deg: int) -> List[str]:
    ...
    for score, day in conditions:
        ...
        if current_date != date:
            ...
            day_name = date.strftime("%A")  # Full day name, e.g. "Thursday"
            output.append(
...
def calculate_favorable_wind_conditions_for_a_segment(
    ...
    """Calculate favorability of wind conditions for a segment"""
    ...
    for day in forecast_data:
        ...
        if favorability > 0:
            overall_segment_favorability = calculate_overall_segment_favorability(
                segment_deg, day, min_wind_speed, direction_tolerance
            )
...
def print_segment_opportunity(segment: KOMSegment, conditions: WindConditions) -> None:
    ...
    formatted_wind_conditions = format_wind_conditions(
        conditions, segment.get_direction_degrees()
    )
